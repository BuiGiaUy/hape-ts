...
export class RegisterService {
  ...
  public async checkEmail(emailDto: EmailDto) {
    const user = await this.usersService.findOne({ emailDto });
    if (user) {
      return true;
    }
    return false;
  ...
  sendMailConfirmEmail(user, key: string): void {
    this.mailerService
      .sendMail({
        ...
        subject: 'Xác nhận email cảu bạn trên Hape.vn',
        text: 'Xác nhận email của bạn trên Hape.vn',
        template: './index',
        context: {
          title: 'Xác nhận email của bạn trên Hape.vn',
          description:
            'Cảm ơn bạn đã đăng ký tài khoản. Vui lòng xác nhận địa chỉ email' +
            user.email +
            'bên dưới và đăng ký nhận bản tin từ Hape để cập nhật tin tức mới nhất.',
