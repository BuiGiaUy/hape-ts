...
export class SearchService {
  ...
  async update(index: string, id: string, body: any, refresh = 'wait_for') {
    delete body.id;
    try {
      const insert = {
        index,
  ...
  async checkExisting(
    ...
    try {
      ...
      } = await this.findBySingleField(index, query);
  ...
  async findByMultiFields({
    ...
    const reqParams = {
      index,
      body: { size, from, query: { bool: { must } } },
    };
    if (sort.length) {
      reqParams.body['sort'] = sort;
    }
  ...
  async findById(index: string, id: string) {
    const { body } = await this.esService.get({ index, id });
    return body;
  }
  async delete(index: string, id: string) {
    const res = await this.esService.delete({ index, id, refresh: 'wait_for' });
    return res;
  }
